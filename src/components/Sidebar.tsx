...
// src/components/Sidebar.tsx
import { Link, useLocation } from "react-router-dom";
import { Home, TrendingUp, Gamepad2, Bookmark, Heart, X, Settings, HelpCircle, MessageSquare, Zap } from "lucide-react";
import { useTheme } from "../context/ThemeContext";
import React from "react";
...
const Sidebar = ({ isOpen, closeSidebar }: SidebarProps) => {
  ...
  const navLinks = [
    ...
    { to: "/favorites", icon: <Heart size={20} />, label: "Favorites" },
    { to: "/saved-videos", icon: <Bookmark size={20} />, label: "Saved Videos" },
    { to: "/premium", icon: <Zap size={20} />, label: "YouTube Premium" },
  ];

  const secondaryLinks = [
    { to: "/settings", icon: <Settings size={20} />, label: "Settings" },
    { to: "/help", icon: <HelpCircle size={20} />, label: "Help" },
    { to: "/feedback", icon: <MessageSquare size={20} />, label: "Send Feedback" },
  ];
